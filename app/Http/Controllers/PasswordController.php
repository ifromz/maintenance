...
use App\Validators\Login\RequestResetValidator;
...
class PasswordController extends Controller
{
    ...
    {
        return view('auth.login.password.request');
    }
    ...
    public function postRequest()
    {
        if ($this->requestResetValidator->passes()) {
            ...
            if ($user) {
                $sent = $this->mail->send('emails.reset-password', [
                    'user' => $user,
    ...
    public function getReset($id, $key)
    {
        ...
        if ($user->checkResetPasswordCode($key)) {
            ...
            $this->redirect = route('maintenance.login.index');
